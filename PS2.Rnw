...
From the result of m2 and m3 in table 2, we can see income has a statistically significant effect on non-Democrats feeling (coefficient:-0.08521;p-value:4.12e-13). But the siginificance of coefficient of income is not shown in m3 (coefficient:0.01925; p-value:0.3032), meaning that the income does not have signficant influence on feeling of Democrats.  
...
<<label=Answer4>>=
##Erico: I am confused here. Based on the question, which coefficient we should run a simulation for:Coef.income or Coef.nondem (or Coef.dem if we use m2)?

set.seed(740)
n.sims<-1988
m3.sims <- sim(m3, n.sims)
apply(m3.sims@coef, 2, mean)


coef.no.intercept<-m3.sims@coef[,2:8]
coef.income<-data.frame(fake_nondem=seq(min(obama$nondem),max(obama$nondem),length.out=100),coef_income=NA, ub_income=NA, lb_income=NA)

for(i in 1:100){coef.income$coef_income[i]<-mean(m3.sims@coef[,])}

for(i in 1:100) {   
    coef.income$coef_income[i] <- mean(m3.sims@coef[,2] + coef.income$fake_nondem[i]*m3.sims@coef[,8])
    coef.income$ub_income[i] <- quantile(m3.sims@coef[,2] + coef.income$fake_nondem[i]*m3.sims@coef[,8], .975)
    coef.income$lb_income[i] <- quantile(m3.sims@coef[,2] + coef.income$fake_nondem[i]*m3.sims@coef[,8], .025)    
}

View(coef.income)
income.coef.plot<-ggplot(coef.income,aes(x=fake_nondem,y=coef_income))+
        geom_line()+geom_ribbon(aes(ymin=lb_income,ymax=ub_income),alpha=.5)+ 
                                 xlab("Democrat")+
                                 ylab("Coefficient for Income")+
